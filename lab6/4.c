...
void function(int *arr, int rows, int cols) {
    ...
    for (i = 0; i < rows; i++) {
        for (j = 0; j < cols; j++) {
            if (arr[i * cols + j] < arr[i * cols + (j + 1) % cols]    //Праворуч
             && arr[i * cols + j] < arr[i * cols + (j - 1) % cols]    //Ліворуч
             && arr[i * cols + j] < arr[((i + 1) % rows) * cols + j]  //Знизу
             && arr[i * cols + j] < arr[((i - 1) % rows) * cols + j]  //Зверху
             && arr[i * cols + j] < arr[((i + 1) % rows) * cols + (j + 1) % cols]  //Знизу праворуч
             && arr[i * cols + j] < arr[((i + 1) % rows) * cols + (j - 1) % cols]  //Знизу ліворуч
             && arr[i * cols + j] < arr[((i - 1) % rows) * cols + (j + 1) % cols]  //Зверху праворуч
             && arr[i * cols + j] < arr[((i - 1) % rows) * cols + (j - 1) % cols]) { //Зверху ліворуч
                arr[i * cols + j] = 0;
             }
        }
    }
